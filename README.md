...
Аналогично можно одной командой наполнить базу демонстрационными данными
(учётки пациентов/врачей/администратора, паспорта, медицинские записи,
комментарии, вложения и полный sharing-flow):

```bash
docker run --rm --env-file .env docere-api:latest seed-demo
```

Команда идемпотентна: ранее созданные демо-данные удаляются перед вставкой,
поэтому повторный запуск приводит базу к одному и тому же состоянию.
Пароль для всех демо-учёток по умолчанию — `DemoPass123` (можно переопределить
флагом `--password`). Локально без Docker: `PYTHONPATH=src uv run python -m app.presentation.cli seed-demo`.
