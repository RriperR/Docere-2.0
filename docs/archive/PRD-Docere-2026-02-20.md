...
6. Sharing поддерживает принятие, отклонение, отмену pending-запроса отправителем и отзыв ранее принятого доступа отправителем.
...
6. Пациент может иметь в кабинете карточку другого пациента, если ему дали доступ к записи в этом контексте.
7. Конфликт нескольких `UserRecordLink` на одну запись решается правилом приоритета отображения.
8. При `declined` ссылка не создается.
9. Отправитель может отменить pending-запрос (`cancelled`) до ответа получателя.
10. Отправитель может отозвать ранее принятый share (`revoked`); связанные `UserRecordLink`, созданные этим share, удаляются.
...
2. При `accepted` записи пациентом система создает `UserRecordLink` этой записи в контексте `PatientPassport`, с которым запись была доступна отправителю.
3. Отдельного сценария \"принять/подтвердить паспорт\" в MVP нет.
4. Для пациента после `accepted` запись может отображаться в карточке другого пациента, если share был отправлен в этом контексте.
5. Сценарий конкурирующих `UserRecordLink` для одной записи учитывается правилом приоритета отображения.
...
6. При `declined` связь не создается.
...
### FR-SHARE-3 Cancel и revoke

Acceptance criteria:

1. Отправитель может отменить pending share-запрос.
2. Отправитель может отозвать accepted share-запрос.
3. При revoke удаляется фактический доступ получателя, выданный этим share.
4. Статусы share: `pending/accepted/declined/cancelled/revoked`.
